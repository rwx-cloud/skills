...
The `rwx` CLI must be on your PATH. Install it from [RWX docs](https://www.rwx.com/docs/rwx/getting-started/installing-the-cli).

## Available Skills

### migrate-from-gha

Migrates a GitHub Actions workflow to an optimized RWX config with DAG parallelism, content-based caching, and RWX packages.
...
After writing or modifying RWX config files (`.rwx/*.yml`), validate them by running `rwx lint .rwx/<name>.yml`
...
Each skill directory also contains a `references/` folder with pointers to the latest RWX documentation.
